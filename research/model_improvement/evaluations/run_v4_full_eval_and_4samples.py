"""
Automated Comprehensive Evaluation Pipeline for Candidate Model V4.
Evaluates Candidate V4 across:
...
CANDIDATE_V4_PATH = ROOT / "research" / "model_improvement" / "checkpoints" / "best_candidate_model_v4.pth"

SAMPLES_DIR = ROOT / "samples"
UNSEEN_DIR = ROOT / "samples" / "unseen_samples" / "VIDEO_CLIPS_JABEZ_UNSEEN" / "VIDEO CLIPS"

OUT_DIR_4SAMPLES = ROOT / "research" / "model_improvement" / "evaluations" / "v4_4samples_outputs"
OUT_DIR_10CLIPS = ROOT / "research" / "model_improvement" / "evaluations" / "v4_10clips_outputs"
VISUAL_DIFFS_DIR = ROOT / "research" / "model_improvement" / "visual_comparisons_v4"
...
def load_models(device="cuda"):
    print("Loading Model 9 baseline and Candidate Model V4...")
    # Model 9
    ...
    # Candidate V4
    ckpt4 = torch.load(CANDIDATE_V4_PATH, map_location=device)
    sizes = ckpt4.get("anchor_sizes", ((12,), (20,), (36,), (64,), (96,)))
    ratios = ckpt4.get("anchor_aspect_ratios", (0.5, 0.7, 1.0, 1.4, 2.0))
    m4 = build_research_model(num_classes=3, anchor_sizes=sizes, aspect_ratios=ratios, pretrained_backbone=False)
    m4_state = ckpt4["model_state_dict"] if "model_state_dict" in ckpt4 else ckpt4
    m4.load_state_dict(m4_state)
    m4.to(device).eval()
...
def run_video_inference(vpath, m9, m4, out_dir, diff_dir, conf_thresh=0.50, device="cuda"):
    ...
    annotated_out_path = out_dir / f"{vname_clean}_v4_annotated.mp4"
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    writer = cv2.VideoWriter(str(annotated_out_path), fourcc, fps, (w, h))

    frames_dir = out_dir / f"{vname_clean}_v4_detected_frames"
    frames_dir.mkdir(parents=True, exist_ok=True)

    m9_detections_total = 0
    m4_detections_total = 0
    diff_records = []
    ...
    while True:
        ...
        b4 = [b for b, s in zip(o4["boxes"].cpu().numpy(), o4["scores"].cpu().numpy()) if s >= conf_thresh]
        s4 = [s for s in o4["scores"].cpu().numpy() if s >= conf_thresh]
        l4 = [l for l, s in zip(o4["labels"].cpu().numpy(), o4["scores"].cpu().numpy()) if s >= conf_thresh]

        m9_detections_total += len(b9)
        m4_detections_total += len(b4)

        # Draw Candidate V4 detections for annotated output video
        annotated_frame = draw_header_and_boxes(frame, b4, s4, l4, "CANDIDATE MODEL V4")
        writer.write(annotated_frame)

        if len(b4) > 0 and frame_idx % 5 == 0:
            frame_save_path = frames_dir / f"frame_{frame_idx:05d}_v4_detected.jpg"
            cv2.imwrite(str(frame_save_path), annotated_frame)

        # Check for meaningful difference between M9 and V4
        has_diff = False
        if len(b9) != len(b4):
            has_diff = True
        elif len(b9) > 0 and len(b4) > 0:
            if abs(max(s4) - max(s9)) > 0.15:
                has_diff = True

        if has_diff and len(diff_records) < 6:
            f9_ren = draw_header_and_boxes(frame, b9, s9, l9, "MODEL 9 SOTA (BASELINE)")
            f4_ren = draw_header_and_boxes(frame, b4, s4, l4, "CANDIDATE MODEL V4")
            side_by_side = np.hstack([f9_ren, f4_ren])
            ...
            diff_records.append({
                ...
                "v4": [(NAMES.get(l, l), float(f"{s:.3f}"), [int(v) for v in b]) for b, s, l in zip(b4, s4, l4)],
            })
    ...
    return {
        ...
        "candidate_v4_total_detections": m4_detections_total,
...
def main():
    ...
    if not CANDIDATE_V4_PATH.exists():
        print(f"[!] Checkpoint {CANDIDATE_V4_PATH} not found. Awaiting training completion.")
        return

    m9, m4 = load_models(device)

    # 1. Evaluate on 4 Standard Samples
    print("\n" + "=" * 80)
    print("   RUNNING CANDIDATE V4 ON 4 STANDARD MANUSCRIPT SAMPLES")
    print("=" * 80)
    ...
    for item in FOUR_SAMPLE_JOBS:
        ...
        t0 = time.time()
        res = run_video_inference(vpath, m9, m4, OUT_DIR_4SAMPLES, VISUAL_DIFFS_DIR, device=device)
        print(f"   Done in {time.time()-t0:.1f}s | M9 Detections: {res['model9_total_detections']} | V4 Detections: {res['candidate_v4_total_detections']}")
        four_samples_results.append(res)

    out_4s_json = ROOT / "research" / "model_improvement" / "evaluations" / "v4_4samples_summary.json"
    out_4s_json.write_text(json.dumps(four_samples_results, indent=2))

    # 2. Evaluate on 10 Unseen CCTV Clips
    print("\n" + "=" * 80)
    print("   RUNNING CANDIDATE V4 ON 10 UNSEEN CCTV CLIPS")
    print("=" * 80)
    ...
    for idx, vpath in enumerate(videos, start=1):
        ...
        t0 = time.time()
        res = run_video_inference(vpath, m9, m4, OUT_DIR_10CLIPS, VISUAL_DIFFS_DIR, device=device)
        print(f"   Done in {time.time()-t0:.1f}s | M9 Detections: {res['model9_total_detections']} | V4 Detections: {res['candidate_v4_total_detections']}")
        ten_clips_results.append(res)

    out_10c_json = ROOT / "research" / "model_improvement" / "evaluations" / "v4_10clips_summary.json"
    out_10c_json.write_text(json.dumps(ten_clips_results, indent=2))

    print("\n" + "=" * 80)
    print("ALL CANDIDATE V4 EVALUATIONS COMPLETED!")
