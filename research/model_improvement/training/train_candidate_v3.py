"""
Candidate Model V3 Training Pipeline.
Focus:
1. Physical Scale Capping (max anchor 96px, aspect ratios (0.5, 0.7, 1.0, 1.4, 2.0))
   Mathematically eliminates >135px architectural proposals (counter tops, refrigerator kicks).
2. Handgun feature preservation via early backbone freezing (conv1, bn1, layer1).
3. Transfer learning from Model 9 SOTA baseline.
Outputs strictly saved to research/model_improvement/checkpoints/best_candidate_model_v3.pth.
"""
import os
import sys
import time
...
BEST_CHECKPOINT = OUTPUT_DIR / "best_candidate_model_v3.pth"
LATEST_CHECKPOINT = OUTPUT_DIR / "latest_candidate_model_v3.pth"
...
def load_model9_weights(model, checkpoint_path):
    ...
    print(f"  -> Transferred {len(filtered)} layers cleanly from Model 9.")
    print(f"  -> Fresh RPN Head initialized for calibrated 5 aspect ratios: {[k for k in missing if 'rpn' in k]}")
    return model


def parse_args():
    parser = argparse.ArgumentParser(description="Train Candidate Model V3 with Physical Scale Capping")
    ...
    parser.add_argument("--lr-backbone", type=float, default=0.0004, help="Fine-tuning LR for backbone (default: 0.0004)")
    parser.add_argument("--lr-rpn", type=float, default=0.002, help="Learning rate for new RPN head (default: 0.002)")
...
def main():
    ...
    print("\n" + "=" * 76)
    print("   RESEARCH PIPELINE: CANDIDATE MODEL V3 (PHYSICAL SCALE CAPPING)")
    print("=" * 76)
    ...
    print(f"Aspect Ratios:          {ASPECT_RATIOS} [Eliminates Extreme Architectural Ratios]")
    print(f"Scales:                 {ANCHOR_SIZES} [Capped at 96px - Max box 135px]")
    print(f"Backbone Early Freezing: Enabled (conv1, bn1, layer1 preserved)")
    ...
    # 3. Trainable Parameters (excluding frozen early backbone)
    ...
    print("\n" + "=" * 76)
    print(f"CANDIDATE V3 TRAINING COMPLETE IN {total_time/60:.2f} MINUTES!")
