...
class SeventhModelDataset(Dataset):
    def __init__(self, split_name, augment=True):
        ...
        data_root = root / "seventh_model_data"
    ...
    def _apply_surveillance_augmentations(self, image, boxes, is_knife):
        ...
        # 4. CCTV Downsampling Simulation
        ...
        # 5. Synthetic Infrared (IR) / Night-Vision Simulation (25% probability)
        # Bridges the RGB -> IR monochrome domain gap for night-vision CCTV cameras
        if random.random() < 0.25:
            gray = np.array(image.convert("L"))
            # Monochromatic sensor simulation with slight phosphor tint
    ...
    def __getitem__(self, index):
        ...
        if boxes:
            ...
            # Negative frame (VIRAT, USRT No_Gun, or scene background)
            target = {
